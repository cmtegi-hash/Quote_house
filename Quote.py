...
if st.session_state.final_report:
    ...
    """, height=70)import streamlit as st
import streamlit.components.v1 as components
import re
import json
import uuid
...
if st.session_state.final_report:
    ...
    components.html(f"""<button id="copyR" style="width:100%;padding:10px;background:#2196F3;color:white;border:none;border-radius:5px;cursor:pointer;font-weight:bold;font-family:sans-serif;">Copy Final Report</button>
        <script>document.getElementById("copyR").onclick = () => {{ navigator.clipboard.writeText(`{st.session_state['final_report'].replace('`','\\`').replace('$', '\\$')}`); document.getElementById("copyR").innerText = "✅ Copied!"; }};</script>""", height=50)import streamlit as st
import streamlit.components.v1 as components
import re
